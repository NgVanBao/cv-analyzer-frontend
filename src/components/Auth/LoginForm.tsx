  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Cập nhật AuthContext thay vì mock
        // Ánh xạ dữ liệu từ backend (Vaitro, HoTen, Email) sang object User của frontend
        login({
          id: data.user.MaTaiKhoan.toString(),
          email: data.user.Email,
          name: data.user.HoTen,
          role: data.user.Vaitro === 'Admin' || data.user.Vaitro === 'admin' ? 'admin' : 'user'
        });

        // Điều hướng dựa vào vai trò
        if (data.user.Vaitro === 'Admin' || data.user.Vaitro === 'admin') {
          navigate('/admin/dashboard');
        } else if (data.user.Vaitro === 'UngVien' || data.user.Vaitro === 'ungvien') {
          // Việc navigate sẽ làm unmount Home component và AuthModal tự động đóng
          navigate('/candidate/dashboard');
        } else if (data.user.Vaitro === 'NhaTuyenDung') {
          // Tạm thời nếu bạn có trang riêng cho Nhà tuyển dụng thì mở comment ra nhé:
          // navigate('/employer/dashboard');
          alert("Chào mừng Nhà tuyển dụng!");
        } else {
          // Phòng hờ các role khác
          navigate('/candidate/dashboard');
        }
      } else {
